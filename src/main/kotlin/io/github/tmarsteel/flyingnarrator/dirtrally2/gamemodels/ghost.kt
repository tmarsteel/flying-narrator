...
private inline fun ByteBuffer.skipUntil(
...
private fun ByteBuffer.readVector3(): Vector3 {
    return Vector3(
        getFloat().toDouble(),
        getFloat().toDouble(),
        getFloat().toDouble(),
...
private fun ByteBuffer.readDelimitedBigIntAssumeMax64Bits(): ULong {
    val size = get().toUInt()
    return when(size) {
        1u -> get().toULong()
        2u -> getShort().toULong()
...
private fun ByteBuffer.asString(charset: Charset): String {
    val bytes = ByteArray(remaining())
    get(bytes)
    return String(bytes, charset)
}

private fun ByteBuffer.readDelimited(): ByteBuffer {
...
private fun ByteBuffer.skipDelimited() {
    val length = get()
    position(position() + length)
}

private fun ByteBuffer.skipNBytes(n: Int) {
    position(position() + n)
}

private fun InputStream.readLEInt64(): ULong {
    val byte1 = read()
    val byte2 = read()
    val byte3 = read()
    val byte4 = read()
